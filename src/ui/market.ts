  ...
  refresh(): void {
    ...
    if (meta.cores.length === 0) {
      ...
      for (const seed of meta.cores) {
        this.coresList.appendChild(
          row(seed, [
            button('입장 (코어 소모)', () => this.actions.useCore(seed)),
            button(`판매 ${appraise(seed)}G`, () => {
              this.actions.sellCore(seed);
              this.refresh();
            }),
            button('코드 복사', () => this.actions.copyCode(seed)),
          ]),
        );
      }
    }

    // 시장 재고 — 드롭 목록 전체 공개가 이 게임의 셀링 포인트
